...
export async function materializeWorkspaceState(home, workspaceId) {
  ...
  const classification = classifyWorkspace(activeClaims);
  const state = {
    version: 1,
    generatedAt: now(),
    workspace: {
    ...
    classification: {
      ...
      secondaryCta: classification.secondaryCta
    },
